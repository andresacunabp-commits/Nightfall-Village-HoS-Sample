...
OUT_DIR = os.path.join(ROOT, "game", "images", "backgrounds", "v103")
MODEL = os.path.join(ROOT, "tools", "FSRCNN_x4.pb")
...
def sharpen(image):
    # Conservative unsharp mask: improves perceived detail without halos.
    blur = cv2.GaussianBlur(image, (0, 0), 0.85)
    return cv2.addWeighted(image, 1.20, blur, -0.20, 0)


def grade(image):
    # Small local-contrast lift. Keep the generated artwork's original palette.
...
def main():
    if not os.path.isfile(MODEL):
        raise RuntimeError("FSRCNN model is missing")
    ...
    for name in FILES:
        ...
        # AI super-resolution first, then a high-quality downsample to the
        # project's native 1280x720 canvas. This avoids stretching tiny JPEGs
        # directly in Ren'Py, which was the source of the visibly blurry art.
        enlarged = sr.upsample(source)
        native = cv2.resize(enlarged, (1280, 720), interpolation=cv2.INTER_LANCZOS4)
        native = grade(native)
        native = sharpen(native)
    ...
    with open(os.path.join(OUT_DIR, ".generated_v0.10.3"), "w", encoding="utf-8") as f:
        f.write("FSRCNN x4 -> 1280x720, JPEG quality 94\n")
